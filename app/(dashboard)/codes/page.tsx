...
import AddCodesAlert from "@/components/codes/addCodesAlert"
...
export default function Codes() {
  ...
  const [columns, setColumns] = React.useState([
    ...
    {
      accessorKey: "codeCode",
      header: "Code",
      show: true,
    },
    {
      accessorKey: "codeDesc",
      header: "Description",
      show: true,
    },
    {
  ...
  const filterSearch = async (searchValue: any, filterType: any) => {
    ...
    if (filterType == "dropdown") {
      ...
      url = `${baseURL}/v1/codes?filter=${searchValue}&page=10&limit=100`
    }
  ...
  return (
    <div className="overscroll-y-none px-2 pb-1 pt-2">
      <div className="dark-container relative h-[calc(100vh-72px)] rounded-lg border bg-white p-2">
        <div className="flex">
          <h2 className="text-l mx-1 mt-0.5 font-bold">Codes</h2>
          <div className="relative ml-1 md:ml-5 h-8">
            <Input
              className="w-24 md:w-44 pl-9 text-xs"
          ...
          <div className="px-2">
            <Select
              ...
              onValueChange={(value: any) => {
                filterSearch(value, "dropdown")
                setFilterValue("")
                setFilterSelectValue(value)
              }}
            >
              <SelectTrigger className="h-8 w-24 md:w-[250px] text-gray-600">
                <SelectValue placeholder="Select Type" />
              </SelectTrigger>
              <SelectContent className="h-[150px]">
                <SelectItem value="">Select All</SelectItem>
                {codeType?.map((map_ele: any, i) => (
                  <SelectItem value={map_ele?.codeType} key={i}>
                    {map_ele?.codeType}
                  </SelectItem>
                ))}
              </SelectContent>
                                  ...
                                  onCheckedChange={(value) => {
                                    if (value) {
                                      ...
                                      if (objArray.length === 1) {
                                        ...
                                        //             onClick={() => deleteCodesData(objArray)}
                                        //           > Delete
                                        //           </Button>
                                        //         </div>
                                        //       </div>
