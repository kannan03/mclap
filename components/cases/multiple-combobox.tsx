...
export function MultipleCombobox({ ListData, handleChange, EditData, viewMode, disabled
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      {selectedValues.length > 0 &&
        selectedValues.map((value, i) => (
            <Badge key={value} variant="outline"  className="pl-3 mb-1 pr-0 mr-2 h-6 whitespace-nowrap rounded-md">
              <span className="pr-2 border-r text-[0.65rem] font-normal"> {ListData.find((map_ele: any) => map_ele.codeDesc === value)?.codeDesc} </span>
      ...
      <PopoverContent
        ...
        <Command className="dark:bg-slate-900">
          <CommandInput placeholder="Search Aggravating Factor" className="h-8 text-xs" />
          <CommandEmpty>No Aggravating Factor found.</CommandEmpty>
            ...
            {ListData?.sort()?.map((map_ele: any) => (
              ...
              >
                <Check
                  className={cn(
                    "mr-2 h-4 w-4",
                    selectedValues.includes(map_ele?.codeDesc) ? "opacity-100" : "opacity-0"
                  )}
                />
                {map_ele.codeDesc}
              </CommandItem>
